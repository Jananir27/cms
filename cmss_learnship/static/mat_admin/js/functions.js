...
$(document).ready(function(){
        ...
        $('body').on('change', '#toggle-width input:checkbox', function () {
            ...
            if ($(this).is(':checked')) {
                setTimeout(function () {
                    ...
                    if($('#mon_alloc_chart')[0]){
                        $('#mon_alloc_chart').highcharts().reflow();
                        $('#sell_chart').highcharts().reflow();
                        $('#adh_chart').highcharts().reflow();
                        $('#stock_chart').highcharts().reflow();
                        $('#total_pie_chart').highcharts().reflow();
                        $('#total_pie_chart1').highcharts().reflow();
                    }
                    if ($('#sortingTable')[0]){
                    $('#sortingTable').resize();
                    }
                    ...
                    if ($('#sorting_tabs_issue')[0]){
                    $('#sorting_tabs_issue').resize();
                    }
                    if($('#sorting_tabs_issue1')[0]){
                    $('#sorting_tabs_issue1').resize();
                    }
                }, 250);
            }
            else {
    ...
    (function(){
        ...
        $('body').on('click', '.sub-menu > a', function(e){
            ...
            $('.sub-menu.toggled').not($dets).each(function(){
                    $(this).removeClass('toggled');
                    $(this).find('ul').hide();
                    $det = $(this)
            });
            $(this).not($dets).next().slideToggle(200);
            $(this).not($dets).parent().toggleClass('toggled');
        });
    ...
    */
    ...
    if ($('.date-pick')[0]) {
        ...
        $('.date-pick').datetimepicker({
            format: 'YYYY-MM-DD', maxDate : 'now',defaultDate : dateStr,
        }); }
